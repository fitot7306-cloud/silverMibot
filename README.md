...
```
silvermibot/                  ← корінь репо (GitHub)
...
2. Надішли `/newbot`
3. Введи назву: `SilverMibot`
4. Введи username: `SilverMibot` (або інший вільний)
...
```bash
# Розпакуй архів
unzip silvermibot.zip
cd silvermibot

# Ініціалізуй git
git init
git add .
git commit -m "init: SilverMibot project"

# Створи репо на github.com (назва: silvermibot)
# Потім:
git remote add origin https://github.com/ТВІЙ_ЮЗЕР/silvermibot.git
git branch -M main
...
3. Авторизуй GitHub → вибери репо `silvermibot`
...
| `BOT_USERNAME` | username бота без @ (наприклад `SilverMibot`) |
...
Отримаєш щось на кшталт: `https://silvermibot-backend-xxxx.up.railway.app`
...
В тому ж Railway проекті → `+ New` → `GitHub Repo` → знову вибери `silvermibot` → folder: **`frontend`**
...
Отримаєш: `https://silvermibot-frontend-xxxx.up.railway.app`
...
3. Введи URL: `https://silvermibot-frontend-xxxx.up.railway.app`
4. Введи текст кнопки: `⚡ Mine`

Або через `/newapp`:
1. `/newapp` → вибери бота
2. Заповни дані → Web App URL: `https://silvermibot-frontend-xxxx.up.railway.app`
